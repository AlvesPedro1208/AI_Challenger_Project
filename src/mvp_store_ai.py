...
from db_oracle import init_db, log_event, log_path, upsert_session, log_customer_object, log_purchase_validation
...
def main():
    ...
    try:
        ...
        for pid, person in persons.items():
            ...
            # Salvar objetos do cliente
            for obj in person.customer_objects:
                log_customer_object(obj['ts'], obj['person_id'], obj['camera_id'],
                                   obj['object_type'], obj['roi_id'], obj['action'], obj['confidence'])
            
            # Salvar validações de compra
            for validation in person.purchase_validations:
                log_purchase_validation(validation['ts'], validation['person_id'], validation['camera_id'],
                                       validation['checkout_id'], validation['predicted_propensity'],
                                       validation['propensity_score'], validation['events_detected'],
                                       validation['actual_purchase'])
